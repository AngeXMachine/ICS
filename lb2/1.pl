...
% Добавляем ее в результирующий список и рекурсивно вызываем предикат для хвоста исходного списка.
even_elements([H|T], [H|Result]) :-
    0 is H mod 2, % Проверяем, что H четное.
    even_elements(T, Result).

% Правило для случая, когда голова исходного списка нечетная. Просто игнорируем ее и рекурсивно вызываем предикат для хвоста.
